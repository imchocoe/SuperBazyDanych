...
--Syntax Error: Incorrect syntax near 'alter'.
--Syntax Error: Incorrect syntax near 'VIEW'.
--
--
---- =============================================
...
---- =============================================
---- order by nie moze zostac uzyte w widokach bez komendy top/offset
--alter VIEW [238507_order] as
--SELECT top (100) percent 
--ProductID,Name,ListPrice,Weight
--FROM SalesLT.Product
--order by ProductID desc
...
begin
    BEGIN TRY
        ...
        insert into SalesLT.Product
        (
            [Name], [ProductNumber], [ProductCategoryID], StandardCost, ListPrice, [SellStartDate], [rowguid], [ModifiedDate]
        )
        values
...
-- Zadanie 3
-- =============================================
--Wyświetlamy marki aut jako kolumny (PIVOT), a potem za pomocą CTE (aby nie tworzyć tabel tymczasowych) od razu zwijamy to z powrotem do wierszy (UNPIVOT).

with TabelaPivot as(
    select * from (select Marka,Sztuki from dbo.BazaSalonuSamochodowego) as Tab
    pivot (sum(Sztuki) for Marka in ([Opel],[Citroen],[Toyota])) as Piv)

    select Marka, Sztuki
    from TabelaPivot
    unpivot (Sztuki for Marka in ([Opel],[Citroen],[Toyota])) as Unpiv
GO
